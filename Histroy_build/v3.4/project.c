...
int game_begin_fun()
{
    ...
    //图片显示 bmpname:要显示的图片名 x0,y0:表示要显示的位置起始坐标 n:缩小图片倍数
    int show_bmp(char *bmpname,int x0,int y0,int n)
    {
        //打开图片
        ...
        for(int y=0;y<480;y++)
        {
            for(int x=0;x<800;x++)
            {
                bmp1[800*y+x]=bmp[(479-y)*800+x];
            }
        }
        //将数据写入到内存映射空间内
        for(int y=0;y<480/n;y++)
        {
            for(int x=0;x<800/n;x++)
            {
                p[(y+y0)*800+x+x0]=bmp1[y*n*800+x*n];
            }
        }
        close(bmp_fd);
        return 0;
    ...
    int game_main()
    {
        ...
        if(n<=9)//百分之10概率
        {
            show_bmp("1.bmp",0,0,1);//涂层
            bmp_rgb("one.bmp");//奖项像素点 一等奖
        }
        if(n>9&&n<=29)//百分之20概率
        {
            show_bmp("1.bmp",0,0,1);//涂层
            bmp_rgb("two.bmp");//奖项像素点 二等奖
        }
        if(n>29&&n<=59)//百分之30概率
        {
            show_bmp("1.bmp",0,0,1);//涂层
            bmp_rgb("three.bmp");//奖项像素点 三等奖
        }
        if(n>59)//百分之40概率
        {
            show_bmp("1.bmp",0,0,1);//图层
            bmp_rgb("xie.bmp");//奖项像素点 谢谢惠顾
        }
